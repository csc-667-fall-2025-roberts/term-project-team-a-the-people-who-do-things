...
io.on("connection", (socket: Socket) => {
	...
	socket.on("join-game", async (gameId: string) => {
		...
		if (!games) {
			...
			// Fetch board tiles from database
			const boardTilesResult = await pool.query(
				"SELECT row, col, letter FROM board_tiles WHERE game_id = $1",
				[gameId],
			);

			// Reconstruct board state from board_tiles
			let boardState: (string | null)[][] | null = null;
			if (boardTilesResult.rows.length > 0) {
				boardState = Array(15).fill(null).map(() => Array(15).fill(null));
				for (const tile of boardTilesResult.rows) {
			...
			const currentTurnUserId = gameResult.rows[0]?.current_turn_user_id;

			// Check again if game was created by another request (race condition)
			games = gameManager.getGame(gameId);
			if (!games) {
				games = gameManager.createGame(gameId, game_participants, boardState);
			}

			// Set current player index based on current_turn_user_id
			if (currentTurnUserId) {
				const currentIndex = game_participants.indexOf(currentTurnUserId);
				if (currentIndex !== -1) {
					games.currentPlayerIndex = currentIndex;
				}
			}

			// Load tile bag from database
			const tileBagResult = await pool.query(
				"SELECT letter FROM tile_bag WHERE game_id = $1",
				[gameId],
			);
			if (tileBagResult.rows.length > 0) {
				games.tileBag = tileBagResult.rows.map((r: any) => r.letter);
			}

			// Load all player hands from database
			for (const playerId of game_participants) {
				...
				if (handResult.rows.length > 0) {
					games.playerHands[playerId] = handResult.rows.map((r: any) => r.letter);
				}
			}
			...
			for (const row of scoresResult.rows) {
				games.scores[row.user_id] = row.value;
			}
		}
	...
	socket.on("make-move", async ({ gameId, tiles, words, scores }) => {
		...
		console.log("Move applied. Player hand after move:", game.getPlayerHand(userId));
		console.log("New tiles drawn:", result.newTiles);
		...
		console.log("Sending new tiles to player:", result.newTiles);
		...
		try {
         ...
         SET value = scores.value + $3`,
	        ...
	        ON CONFLICT (game_id, row, col) DO NOTHING`,
					[gameId, t.row, t.col, t.letter, userId],
				);
			}
        ...
        LIMIT 1
			...
			for (const letter of result.newTiles) {
				await pool.query(
